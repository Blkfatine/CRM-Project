...
def lead_list(request):
    # Récupérer les choix de statut du modèle Lead
    ...
    # Compter les leads pour chaque statut
    statut_counts = {
    ...
    # Récupérer tous les leads et appliquer les filtres
    ...
    # Filtrer les leads non assignés (en attente de suivi)
    leads_pending_followup = Lead.objects.filter(Assigné__isnull=True)

    # Compter les leads pris en charge, non pris en charge, et non contactés
    nombre_leads_pris_en_charge = Lead.objects.filter(Assigné__isnull=False).count()
    nombre_leads_non_pris_en_charge = leads_pending_followup.count()
    nombre_leads_non_contactes = Lead.objects.filter(~Q(statut='Contacté')).count()
    ...
    leads_by_month = (
        Lead.objects.annotate(month=TruncMonth('crée'))
        .values('month')
        .annotate(count=Count('id'))
        .order_by('month')
    )

    # Initialiser les mois et les comptes à zéro
    months = []
    counts = []
    month_names_fr = [
    ...
    # Créer une liste des mois de l'année et initialiser les comptes
    for i in range(1, 13):
        ...
        # Chercher le nombre de leads pour ce mois ou initialiser à 0
        count = next((item['count'] for item in leads_by_month if item['month'].strftime('%B') == month_str), 0)
        counts.append(count)

    # Récupérer les 4 annonces les plus récentes
    announcements = Announcement.objects.all().order_by('-updated_at', '-created_at')[:4]
...
def historique_emails(request, lead_id):
    ...
    emails = Interaction.objects.filter(lead=lead, type='Email').order_by('-date')
    return render(request, 'historique_emails.html', {'emails': emails, 'lead': lead})

def BoiteGmail(request, lead_id):
    lead = get_object_or_404(Lead, pk=lead_id)
    return render(request, 'BoiteGmail.html', {'lead': lead})
...
def send_email(request, lead_id):
    if request.method == 'POST':
        ...
        # Create an Interaction record associated with the lead
        interaction = Interaction.objects.create(
            lead_id=lead_id,
            type='Email',
            sujet=email_subject,  # Assuming 'sujet' is the email subject
            corps=email_body,     # Assuming 'corps' is the email body
            date=timezone.now()    # Current timestamp
        )
        interaction.save()
